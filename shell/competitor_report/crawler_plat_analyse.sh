...
SELECT
  ...
  row_number()
  OVER (PARTITION BY c1.plat
    ORDER BY c1.categories - coalesce(c2.categories, 0) DESC) categories_change_rank,
  c1.new_categories-coalesce(c2.new_categories, 0) new_categories_changed,
  row_number()
  OVER (PARTITION BY c1.plat
    ORDER BY c1.new_categories - coalesce(c2.new_categories, 0) DESC) new_categories_change_rank,
  '$date'
FROM
  (
    SELECT
