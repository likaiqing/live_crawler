            ...
            if (!curUrl.equals("http://chushou.tv/live/list.htm")) {
                ...
                if (items.size() > 0 && cnt++ < Const.CHUSHOUMAX) {
                    ...
                    for (int i = 0; i < read.size(); i++) {
                        ...
                        HttpUtil.sendGet(new StringBuffer(Const.DDPUNCHDOMAIN).append(Const.ANCHOREVENT).append("&par_d=").append(date).append("&rid=").append(JsonPath.read(room, "$.targetKey").toString()).append("&nm=").append(CommonTools.getFormatStr(JsonPath.read(room, "$.meta.creator").toString())).append("&tt=").append(CommonTools.getFormatStr(JsonPath.read(room, "$.name").toString())).append("&cate=").append(JsonPath.read(room, "$.meta.gameName").toString()).append("&pop_s=").append(JsonPath.read(room, "$.meta.onlineCount").toString()).append("&pop_n=").append(CommonTools.createNum(JsonPath.read(room, "$.meta.onlineCount").toString())).append("&task=").append(job).append("&plat=").append(Const.CHUSHOU).append("&url_c=").append(Const.GAMEALL).append("&c_time=").append(createTimeFormat.format(new Date())).append("&url=").append(curUrl).append("&t_ran=").append(PandaProcessor.getRandomStr()).toString());
...
//                        anchorObjs.add(anchor);
                ...
                for (int i = 0; i < rids.size(); i++) {
                    HttpUtil.sendGet(new StringBuffer(Const.DDPUNCHDOMAIN).append(Const.ANCHOREVENT).append("&par_d=").append(date).append("&rid=").append(rids.get(i)).append("&nm=").append(CommonTools.getFormatStr(names.get(i))).append("&tt=").append(CommonTools.getFormatStr(titles.get(i))).append("&cate=").append(categories.get(i)).append("&pop_s=").append(popularitiyStrs.get(i)).append("&pop_n=").append(CommonTools.createNum(popularitiyStrs.get(i))).append("&task=").append(job).append("&plat=").append(Const.CHUSHOU).append("&url_c=").append(Const.GAMEALL).append("&c_time=").append(createTimeFormat.format(new Date())).append("&url=").append(curUrl).append("&t_ran=").append(PandaProcessor.getRandomStr()).toString());
//                    Anchor anchor = new Anchor();
