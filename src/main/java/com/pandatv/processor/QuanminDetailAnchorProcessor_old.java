 ...
 */
public class QuanminDetailAnchorProcessor_old extends PandaProcessor {
    ...
    private static final Logger logger = LoggerFactory.getLogger(QuanminDetailAnchorProcessor_old.class);
...
//                detailAnchorObjs.add(detailAnchor);
            ...
            if (exCnt++ > Const.EXTOTAL) {
                MailTools.sendAlarmmail("chushouanchor 异常请求个数过多", "url: " + curUrl);
                System.exit(1);
            }
        }
    }
    ...
    public static void crawler(String[] args) {
        ...
        Spider.create(new QuanminDetailAnchorProcessor_old()).thread(2).addUrl(firUrl + dateStr).addPipeline(new ConsolePipeline()).setDownloader(new PandaDownloader()).run();
